...
def add_vectors(vector1, vector2):
    x = sin(vector1.angle)*vector1.magnitude + \
        sin(vector2.angle)*vector2.magnitude
    y = cos(vector1.angle)*vector1.magnitude + \
        cos(vector2.angle)*vector2.magnitude
...
class Block:
    def __init__(self, x, y, r, v=None, color=(120, 40, 31), colorBoundary=(28, 40, 51)):
        ...
        self.block_destroyed_image = pygame.image.load(
            "Images/block_destroyed1.png")
...
class Slingshot:
    ...
    def draw(self, loaded=None):
        pygame.draw.rect(display, self.color,
                         (self.x, self.y + self.h*1/3, self.w, self.h*2/3))

        if (not loaded == None) and loaded.loaded:
            pygame.draw.line(display, (100, 30, 22), (self.x - self.w/4 + self.w/4,
                             self.y + self.h/6), (loaded.x, loaded.y + loaded.r/2), 10)
            pygame.draw.line(display, (100, 30, 22), (self.x + self.w, self.y +
                             self.h/6), (loaded.x + loaded.r, loaded.y + loaded.r/2), 10)

        pygame.draw.rect(display, self.color, (self.x -
                         self.w/4, self.y, self.w/2, self.h/3), 5)
        pygame.draw.rect(display, self.color, (self.x + self.w -
                         self.w/4, self.y, self.w/2, self.h/3), 5)
